...
class MobileNetV3(nn.Module):
    def __init__(self, model_mode="LARGE", num_classes=1000, multiplier=1.0, dropout_rate=0.0, hyperargs = None):
        super(MobileNetV3, self).__init__()
        self.num_classes = num_classes
        if hyperargs != None:
            self.in_channels = hyperargs['in_channels']
            self.kernel_size = hyperargs['kernel_size']
            self.stride = hyperargs['stride']
            self.padding = hyperargs['padding']
        ...
        if model_mode == "LARGE":
            ...
            self.init_conv = nn.Sequential(
                nn.Conv2d(in_channels=self.in_channels, out_channels=init_conv_out, kernel_size=self.kernel_size, stride=self.stride, padding=self.padding),
